...
public class Exercises {
	...
	/*
	 ...
	 the parameter vacation is true if we are on
	 vacation. We sleep in if it is not a weekday or we're on vacation. 
	 Return true if we sleep in.
	 sleepIn(false, false) → true
	 sleepIn(true, false) → false
	 sleepIn(false, true) → true
	 */
	public boolean sleepIn(boolean isWeekday, boolean vacation) 
	{
		if(vacation) 
	{
				return true;
	 ...
	 */
	public boolean parrotTrouble(boolean parrotTalking, int hour) 
	{
		if (hour < 7|| hour >20)
	{
		if(parrotTalking)
	...
	public boolean or35(int n) 
	{
		...
		boolean isItMultipuleOfThree= n % 3 == 0;
		boolean isItMultipuleOfFive = n % 5 == 0;
		boolean resultEquals = isItMultipuleOfFive || isItMultipuleOfThree;
