...
public class Exercises {
	...
	{
			boolean firstLastEqual = nums[0] == nums[nums.length-1];
			theResultIs = arrayLengthAtLeastOne && firstLastEqual;
	}
		return theResultIs;
	}
	...
	{
			boolean isElement1Same = a[0] == b[0];
			boolean isElement2Same = a[a.length - 1] == b[b.length -1];
			
			boolean result = isElement1Same || isElement2Same;
	...
	{
		int[] figureLarge = new int[3];
		if(nums[0] > nums[2])
	{
		figureLarge[0] = nums[0];
		figureLarge[1] = nums[0];
		figureLarge[2] = nums[0];
		
		return figureLarge;
	}
		figureLarge[0] = nums[2];
		figureLarge[1] = nums[2];
		figureLarge[2] = nums[2];
	...
	public int sum13(int[] nums) 
	{
		...
		for(int i = 0; i < nums.length; i++)
	{
			countVariable = countVariable + nums[i];
			if(nums[i] == 13)
	{
			countVariable = countVariable - nums[i];
			i = i + 1;
	...
	public boolean has22(int[] nums) 
	{
		for (int i = 0; i < nums.length; i++)
		{
			boolean moreNumbs = nums.length > i + 1;
			if(nums[i] == 2 && moreNumbs && nums[i + 1] == 2)
			{
				return true;
			}
		}
